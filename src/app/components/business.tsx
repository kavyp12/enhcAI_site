...
const ServiceItem = ({ title, description, image, isHovered, onTouch, isAnyHovered }: { title: string, description: string, image: string, isHovered: boolean, onTouch: () => void, isAnyHovered: boolean }) => (
    <div 
        className="relative border-b border-gray-700 py-6 md:py-8 overflow-hidden cursor-pointer" 
    ...
    >
        <div className="flex items-center">
            <h1 className={`text-4xl md:text-8xl font-medium leading-tight transition-all duration-500 ease-out product-sans ${isHovered ? 'md:translate-x-56 sm:translate-x-40 translate-x-32 text-white' : 'translate-x-0'} ${isAnyHovered && !isHovered ? 'text-gray-500' : 'text-white'}`}>
                {title}
            </h1>
        </div>
        
        {/* Mobile image - shows below title */}
        <div className={`block md:hidden mt-4 transition-all duration-500 ease-out ${isHovered ? 'opacity-100 max-h-40' : 'opacity-0 max-h-0'}`}>
            ...
            <p className="mt-3 text-sm text-gray-400 product-sans">{description}</p>
        </div>
        
        {/* Desktop image - shows on the left */}
...
export default function Business() {
  ...
  return (
    <>
      ...
      <section className="bg-[#121212] text-white min-h-screen px-4 sm:px-8 py-16 md:px-20 md:py-24 product-sans">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start mb-20 md:mb-32">
          <div className="flex items-center gap-2 text-sm product-sans">
            <span className="bg-white w-2 h-2 rounded-full"></span>Our Expertise
          </div>
