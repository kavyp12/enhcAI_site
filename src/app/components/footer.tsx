...
export default function Footer() {
  ...
  return (
    <>
      ...
      <footer className="bg-[var(--bg-secondary)] text-[var(--text-main)] product-sans rounded-t-3xl">
        <div className="max-w-screen-2xl mx-auto px-6 lg:px-12">
          <div className="text-center md:text-right pt-10">
            <a href="#" className="text-sm text-[var(--text-muted)] hover:text-[var(--text-main)] transition-colors product-sans">Sh*t I've gone too far, send me back up 👋</a>
          </div>
          <div className="py-16 lg:py-24 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-y-12 lg:gap-x-8">
            <div className="col-span-1 sm:col-span-2 lg:col-span-1">
              <div className="flex justify-center sm:justify-start space-x-2 lg:flex-col lg:space-x-0 lg:space-y-2">
                {socialLinks.map((social, index) => (
                  <a key={index} href={social.href} aria-label={social["aria-label"]} className="w-10 h-10 bg-[var(--bg-card-secondary)] hover:bg-[var(--brand-primary)] text-[var(--text-main)] hover:text-white rounded-full flex items-center justify-center transition-colors"><social.icon className="w-5 h-5" /></a>
                ))}
              </div>
            </div>
            <div className="col-span-1 sm:col-span-2 lg:col-span-4 text-center sm:text-left">
              ...
              <a href="#" className="inline-flex items-center gap-2 bg-[var(--brand-primary)] text-white font-bold py-3 px-6 rounded-full mt-6 hover:bg-white hover:text-black transition-colors product-sans">Start a project<span className="text-lg">↗</span></a>
              <div className="mt-6 flex items-center justify-center sm:justify-start gap-2 text-sm text-[var(--text-muted)]">
                <span className="text-[var(--text-main)] product-sans">5.0 from 69 reviews</span>
            ...
            <div className="col-span-1 sm:col-span-1 lg:col-span-2 text-center sm:text-left">
              ...
              <ul className="space-y-2 text-[var(--text-muted)]">{learnLinks.map((link, index) => (<li key={index}><a href="#" className="hover:text-[var(--text-main)] transition-colors product-sans">{link}</a></li>))}</ul>
            </div>
            <div className="col-span-1 sm:col-span-1 lg:col-span-2 text-center sm:text-left">
              <h3 className="font-bold mb-4 product-sans">Explore</h3>
              <ul className="space-y-2 text-[var(--text-muted)]">{exploreLinks.map((link, index) => (<li key={index}><a href="#" className="hover:text-[var(--text-main)] transition-colors flex items-center justify-center sm:justify-start product-sans">{link.text}{link.isNew && (<span className="ml-2 text-xs font-bold bg-[var(--brand-primary)] text-white px-2 py-0.5 rounded-full product-sans">NEW</span>)}</a></li>))}</ul>
            </div>
            <div className="col-span-1 sm:col-span-2 lg:col-span-2 text-center sm:text-left">
              ...
              <ul className="space-y-4 text-[var(--text-muted)]">{contactDetails.map((item, index) => (<li key={index} className="flex items-start justify-center sm:justify-start gap-3"><span className="mt-1 font-bold text-base">{item.icon}</span><a href={item.href} className="hover:text-[var(--text-main)] transition-colors whitespace-pre-line product-sans">{item.text}</a></li>))}</ul>
            </div>
          </div>
